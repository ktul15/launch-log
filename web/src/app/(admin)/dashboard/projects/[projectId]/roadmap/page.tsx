...
export const metadata: Metadata = { title: 'Roadmap — LaunchLog' }
...
export default async function RoadmapPage({ params }: Props) {
  ...
  const [projectRes, itemsRes] = await Promise.all([
    ...
    fetch(`${BACKEND}/api/v1/projects/${projectId}/roadmap`, {
      headers: { Cookie: cookieHeader },
      cache: 'no-store',
      signal: AbortSignal.timeout(5_000),
    }),
  ...
  if (projectRes.status === 401 || itemsRes.status === 401) redirect('/login')
  if (projectRes.status === 404) redirect('/dashboard/projects')
  if (itemsRes.status === 404) redirect('/dashboard/projects')
  if (!projectRes.ok) throw new Error(`Backend error: ${projectRes.status}`)
  if (!itemsRes.ok) throw new Error(`Backend error: ${itemsRes.status}`)
  ...
  return <RoadmapClient projectId={projectId} projectName={project.name} initialItems={items} />
}
